...
int SDL_SYS_JoystickInit(void)
{
	...
	{
		if (LowLevelBase || (LowLevelBase = OpenLibrary("lowlevel.library",37)))
		{
			numjoysticks = 0;
...
const char *SDL_SYS_JoystickName(int index)
{
	...
	if (LowLevelBase && index < MAX_JOYSTICKS /* && SDL_JoyNames */ )
	{
		unsigned long joyflag;

		joyflag = ReadJoyPort(PortIndex(index));
		...
		if (!(joyflag&JP_TYPE_MASK) == JP_TYPE_NOTAVAIL)
		{
			...
			name = &SDL_JoyNames[index * MAX_JOY_NAME];

			D(bug("Port %ld is %s", index, type));
		}
	}
	else
	{
